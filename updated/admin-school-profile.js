(function () {
  ...
  const schoolUnitSelect = $("schoolUnitId");
  ...
  function currentSchoolUnitId() {
    return schoolUnitSelect ? String(schoolUnitSelect.value || "").trim() : "";
  }

  function withSchoolUnit(url) {
    const id = currentSchoolUnitId();
    if (!id) return url;
    return url + (url.indexOf("?") === -1 ? "?" : "&") + "schoolUnitId=" + encodeURIComponent(id);
  }

  function getTenantCode() {
    if (window.__TENANT_CODE__) return String(window.__TENANT_CODE__).trim();

    if (previewBtn) {
      const dataCode = previewBtn.getAttribute("data-tenant-code");
      if (dataCode) return String(dataCode).trim();

      const href = previewBtn.getAttribute("href") || "";
      const match = href.match(/\/schools\/([^/?#]+)/i);
  ...
  function bindTopActions() {
    ...
    if (previewBtn) {
      previewBtn.addEventListener("click", function (e) {
        ...
        window.open(withSchoolUnit("/schools/" + encodeURIComponent(code)), "_blank", "noopener,noreferrer");
      });
    }
  }

  function bindSchoolUnitSelector() {
    if (!schoolUnitSelect) return;

    schoolUnitSelect.addEventListener("change", function () {
      const id = currentSchoolUnitId();
      const url = id ? BASE + "?schoolUnitId=" + encodeURIComponent(id) : BASE;
    ...
    if (logoFile) {
      logoFile.addEventListener("change", async function (e) {
        try {
          ...
          const data = await uploadSingle(withSchoolUnit(BASE + "/upload/logo"), "logo", f);
    ...
    if (coverFile) {
      coverFile.addEventListener("change", async function (e) {
        try {
          ...
          const data = await uploadSingle(withSchoolUnit(BASE + "/upload/cover"), "cover", f);
      ...
      galleryFiles.addEventListener("change", async function (e) {
        try {
          ...
          async function tryUpload(fieldName) {
            ...
            const res = await fetch(withSchoolUnit(BASE + "/upload/gallery"), {
              method: "POST",
              body: fd,
              headers: { "X-Requested-With": "XMLHttpRequest" }
            });
    ...
    Array.prototype.slice.call(document.querySelectorAll("[data-del]")).forEach(function (btn) {
      btn.addEventListener("click", async function () {
        ...
        try {
          ...
          await delReq(withSchoolUnit(BASE + "/gallery/" + id));
          location.reload();
        } catch (err) {
  ...
  function bindReviewActions() {
    Array.prototype.slice.call(document.querySelectorAll("[data-approve]")).forEach(function (btn) {
      btn.addEventListener("click", async function () {
        try {
          await post(withSchoolUnit(BASE + "/reviews/" + btn.getAttribute("data-approve") + "/approve"));
          location.reload();
        } catch (err) {
    ...
    Array.prototype.slice.call(document.querySelectorAll("[data-reject]")).forEach(function (btn) {
      btn.addEventListener("click", async function () {
        try {
          await post(withSchoolUnit(BASE + "/reviews/" + btn.getAttribute("data-reject") + "/reject"));
          location.reload();
        } catch (err) {
          alert(err.message || "Failed");
        }
    ...
    Array.prototype.slice.call(document.querySelectorAll("[data-feature]")).forEach(function (btn) {
      btn.addEventListener("click", async function () {
        try {
          await post(withSchoolUnit(BASE + "/reviews/" + btn.getAttribute("data-feature") + "/feature"));
          location.reload();
        } catch (err) {
          alert(err.message || "Failed");
        }
    ...
    Array.prototype.slice.call(document.querySelectorAll("[data-delreview]")).forEach(function (btn) {
      btn.addEventListener("click", async function () {
        ...
        try {
          await delReq(withSchoolUnit(BASE + "/reviews/" + btn.getAttribute("data-delreview")));
          location.reload();
        } catch (err) {
          alert(err.message || "Failed");
        }
  ...
  function bindFaqActions() {
    ...
    if (faqFormModal) {
      faqFormModal.addEventListener("submit", async function (e) {
        ...
        try {
          ...
          if (id) {
            await fetchJson(withSchoolUnit(BASE + "/faqs/" + id), {
              method: "PUT",
              headers: {
            ...
            await fetchJson(withSchoolUnit(BASE + "/faqs"), {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                "X-Requested-With": "XMLHttpRequest"
    ...
    Array.prototype.slice.call(document.querySelectorAll("[data-faqdel]")).forEach(function (btn) {
      btn.addEventListener("click", async function () {
        ...
        try {
          await delReq(withSchoolUnit(BASE + "/faqs/" + btn.getAttribute("data-faqdel")));
          location.reload();
        } catch (err) {
          alert(err.message || "Failed");
        }
