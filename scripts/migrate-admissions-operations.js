...
async function main() {
  ...
  for (const tenant of selected) {
    ...
    const result = await migrateAdmissionsOperations(loadTenantModels(conn));
    console.log(`${tenant.code || tenant.dbName}:`, JSON.stringify(result));
  }
...
main().catch(async (err) => {
  console.error('Admissions operations migration failed:', err.message || err);
