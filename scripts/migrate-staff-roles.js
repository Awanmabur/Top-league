...
async function main() {
  ...
  for (const tenant of selected) {
    ...
    const result = await migrateStaffRoles(models);
...
main().catch(async (err) => { console.error("Staff/RBAC migration failed:", err.message || err); await platformConnection.close().catch(() => {}); await mongoose.disconnect().catch(() => {}); process.exit(1); });
