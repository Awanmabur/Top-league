...
async function main() {
  ...
  for (const tenant of selected) {
    ...
    conns.push(conn);
    const lazy = loadTenantModels(conn); const models = {};
    for (const name of Object.keys(lazy)) models[name] = lazy[name];
    console.log(`[ok] ${tenant.code || tenant.dbName}:`, await migratePromotions(models));
  }
...
main().catch(async (err) => {
  console.error('Promotion migration failed:', err.message || err);
