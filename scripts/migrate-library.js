...
async function main(){
  ...
  const conns=[];
  for(const tenant of selected){ if(!tenant.dbName) continue; const conn=await getTenantConnection(tenant.dbName); conns.push(conn); const lazy=loadTenantModels(conn); const models={}; for(const name of Object.keys(lazy)) models[name]=lazy[name]; const r=await migrateLibrary(models); console.log(`[ok] ${tenant.code||tenant.dbName}:`,r); }
  await Promise.allSettled(conns.map(c=>c.close())); await platformConnection.close(); await mongoose.disconnect().catch(()=>{});
}
main().catch(async(err)=>{console.error("Library migration failed:",err.message||err);await platformConnection.close().catch(()=>{});await mongoose.disconnect().catch(()=>{});process.exit(1);});
