...
async function main() {
  ...
  for (const tenant of selected) {
    ...
    const lazy = loadTenantModels(connection);
    const models = {};
    for (const name of Object.keys(lazy)) models[name] = lazy[name];
    console.log(`[ok] ${tenant.code || tenant.dbName}:`, await migrateTranscripts(models));
  }
...
main().catch(async (err) => {
  console.error("Transcript migration failed:", err.message || err);
