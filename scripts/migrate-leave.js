...
async function main() {
  ...
  const tenants = await Tenant.find({ isDeleted: { $ne: true } }).select('name code subdomain dbName timezone').lean();
  ...
  for (const tenant of selected) {
    ...
    const result = await migrateLeaveRequests(models, { timezone: tenant.timezone || 'UTC' });
...
main().catch(async (err) => { console.error('Leave migration failed:', err.message || err); await platformConnection.close().catch(() => {}); await mongoose.disconnect().catch(() => {}); process.exit(1); });
