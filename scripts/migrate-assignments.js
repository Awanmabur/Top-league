...
async function main() {
  ...
  for (const tenant of selected) {
    ...
    const connection=await getTenantConnection(tenant.dbName); connections.push(connection);
    const lazy=loadTenantModels(connection); const models={}; for(const name of Object.keys(lazy))models[name]=lazy[name];
    console.log(`[ok] ${tenant.code||tenant.dbName}:`, await migrateAssignments(models));
  }
  await Promise.allSettled(connections.map((c)=>c.close())); await platformConnection.close(); await mongoose.disconnect().catch(()=>{});
}
main().catch(async(err)=>{console.error("Assignment migration failed:",err.message||err);await platformConnection.close().catch(()=>{});await mongoose.disconnect().catch(()=>{});process.exit(1);});
