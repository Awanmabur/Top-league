...
(function () {
  ...
  function statusPill(s) {
    if (s === "Active") return '<span class="pill ok"><i class="fa-solid fa-check"></i> Active</span>';
    if (s === "On Leave") return '<span class="pill warn"><i class="fa-solid fa-calendar-check"></i> On Leave</span>';
    if (s === "Suspended") return '<span class="pill bad"><i class="fa-solid fa-ban"></i> Suspended</span>';
  ...
  function render() {
    ...
    $("tbody").innerHTML = DATA.map((s) => {
      ...
      return `
        <tr data-id="${s.id}">
          <td><input type="checkbox" class="rowCheck" data-id="${s.id}" ${checked}></td>
          <td>
            <div class="strong">${s.fullName || ""}</div>
            <div class="muted">${s.email || "—"} • ${s.phone || "—"}</div>
          </td>
          <td><span class="pill info"><i class="fa-solid fa-id-badge"></i> ${s.employeeId || "—"}</span></td>
          <td>${s.departmentName || "—"}</td>
          <td>
            <div class="strong">${s.roleName || "—"}</div>
            <div class="muted">${s.jobTitle || "—"}</div>
          </td>
          <td><span class="pill info"><i class="fa-solid fa-briefcase"></i> ${s.employmentType || "—"}</span></td>
      ...
      `;
    }).join("") || '<tr><td colspan="9" style="padding:18px;"><div class="muted">No staff found.</div></td></tr>';
  ...
  function fillForm(s) {
    ...
    form.action = s ? `/admin/staff/${s.id}/update` : "/admin/staff";

    $("fUserId").value = s?.userId || "";
    $("fEmployeeId").value = s?.employeeId === "—" ? "" : (s?.employeeId || "");
    $("fFirstName").value = s?.firstName || "";
    $("fMiddleName").value = s?.middleName || "";
    $("fLastName").value = s?.lastName || "";
    $("fEmail").value = s?.email || "";
    $("fPhone").value = s?.phone || "";
    $("fGender").value = s?.gender || "";
    $("fDepartmentId").value = s?.departmentId || "";
    $("fRoleId").value = s?.roleId || "";
    $("fJobTitle").value = s?.jobTitle || "";
    $("fEmploymentType").value = s?.employmentType || "Full Time";
    $("fJoinDate").value = s?.joinDate || "";
    $("fSalary").value = s?.salary || 0;
    $("fPayrollNumber").value = s?.payrollNumber || "";
    $("fBankName").value = s?.bankName || "";
    $("fBankAccountName").value = s?.bankAccountName || "";
    $("fBankAccountNumber").value = s?.bankAccountNumber || "";
    $("fStatus").value = s?.status || "Active";
    $("fAddress").value = s?.address || "";
    $("fEmergencyContactName").value = s?.emergencyContactName || "";
    $("fEmergencyContactPhone").value = s?.emergencyContactPhone || "";
  ...
  $("tbody").addEventListener("click", function (e) {
    ...
    if (e.target.closest(".actActive")) return submitRowAction(`/admin/staff/${s.id}/status`, { status: "Active" });
    if (e.target.closest(".actLeave")) return submitRowAction(`/admin/staff/${s.id}/status`, { status: "On Leave" });
    if (e.target.closest(".actSuspend")) return submitRowAction(`/admin/staff/${s.id}/status`, { status: "Suspended" });
    if (e.target.closest(".actDelete")) {
      if (window.confirm(`Delete "${s.fullName}"?`)) {
        return submitRowAction(`/admin/staff/${s.id}/delete`);
      }
    }
  });
  ...
  $("bulkLeave").addEventListener("click", function () { bulkSubmit("leave"); });
  $("bulkSuspend").addEventListener("click", function () { bulkSubmit("suspend"); });
  $("bulkExit").addEventListener("click", function () { bulkSubmit("exit"); });
  $("bulkDelete").addEventListener("click", function () {
    if (window.confirm("Delete selected staff records?")) bulkSubmit("delete");
  });
