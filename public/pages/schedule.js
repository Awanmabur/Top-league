        ...
        // ✅ Column highlight aligned with calendar columns
        function setColumnPillForDate(iso) {
            if (!iso) { colPill.style.opacity = 0; return; }

            const [y, m, d] = iso.split("-").map(Number);
            const dt = new Date(y, m - 1, d);
            const colIndex = dt.getDay(); // Sun=0..Sat=6

            const gapStr = getComputedStyle(daysEl).gap || "12px";
            const gap = parseFloat(gapStr.split(" ")[0]) || 12;

            const wrap = daysEl.parentElement; // calGridWrap
            const innerWidth = wrap.clientWidth - 36; // 18 + 18 padding
