...
function friendlyUploadError(err, fallback) {
  ...
  if (code === "LIMIT_FILE_SIZE" || msg.includes("file too large") || msg.includes("payload too large")) {
...
function ensureTenantShape(tenantDoc) {
  ...
  profile.stats.programs = Number(profile.stats.programs || 0);
...
async function getStatsFromTenantDB(tenantDoc) {
  try {
    ...
    return {
      ...
      programs: tenantDoc.settings?.profile?.stats?.programs || 0,
...
function buildProfileStats(tenantDoc) {
  ...
  return {
    ...
    programs: Number(p.stats?.programs || 0),
...
module.exports = {
  page: async (req, res) => {
    try {
      ...
      return res.render("tenant/admin/profile/index", {
        ...
        success: req.query.success ? "Profile saved successfully ✅" : null,
        error: null,
  ...
  update: async (req, res) => {
    try {
      ...
      // optional legacy mirror for easy public use
