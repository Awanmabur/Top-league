...
function applicantBaseFilter({ q, program, status }) {
  ...
  if (program && isValidId(program)) filter.program1 = program;
...
function buildErrors(body, files) {
  ...
  const p1 = (body?.program1 || body?.programId || "").toString().trim();
  if (!p1) e.program1 = "First choice program is required";
  ...
  if (Number.isNaN(sem) || sem < 0 || sem > 6) e.semester = "Semester must be 0–6";
...
async function getApplicantListing(req) {
  const { Applicant, Program } = req.models;

  const q = str(req.query.q);
  const program = str(req.query.program);
  const status = str(req.query.status);

  const page = Math.max(parseInt(req.query.page || "1", 10), 1);
  const perPage = 20;

  const filter = applicantBaseFilter({ q, program, status });
  ...
  return {
    ...
    query: { q, program, status, page: safePage, total, totalPages, perPage },
  };
}
...
module.exports = {
  ...
  exportApplicantsCsv: async (req, res) => {
    try {
      ...
      const csvRows = [
        ["ApplicationId", "FullName", "Email", "Phone", "ProgramCode", "ProgramName", "Intake", "Status", "SubmittedAt"],
        ...rows.map((a) => [
          a.applicationId || "",
          ...
          a.phone || "",
          a.program1?.code || "",
          a.program1?.name || a.program1?.title || "",
          a.intake || "",
          a.status || "",
  ...
  importApplicantsCsv: async (req, res) => {
    try {
      ...
      for (const raw of rows) {
        ...
        const programCode = str(raw.programCode).toUpperCase();
        ...
        const program = await Program.findOne({ code: programCode, isDeleted: { $ne: true } })
          .select("_id")
          .lean();
      ...
      if (skipped) {
        req.flash?.("error", `${skipped} row(s) were skipped due to missing fields, duplicates, or invalid program code.`);
      }
      return res.redirect("/admin/admissions/applicants");
    } catch (err) {
  ...
  viewApplicant: async (req, res) => {
    const { Applicant, Class } = req.models;
    ...
    const applicant = await Applicant.findOne({
      ...
      .populate("program1", "code name title")
      .populate("program2", "code name title")
  ...
  submitApplication: async (req, res) => {
    ...
    try {
      ...
      const program1 = str(req.body.program1 || req.body.programId);
      const program2 = str(req.body.program2 || req.body.program2Id);
  ...
  acceptApplicant: async (req, res) => {
    ...
    const classGroup = str(req.body.classGroup || applicant.preferredClassGroup || "");
    if (!classGroup) {
      return res.redirect(`/admin/admissions/applicants/${req.params.id}?err=class_required`);
    }
    if (!mongoose.Types.ObjectId.isValid(classGroup)) {
      return res.redirect(`/admin/admissions/applicants/${req.params.id}?err=class_invalid`);
    }

    if (!applicant.program1 || !mongoose.Types.ObjectId.isValid(String(applicant.program1))) {
      return res.redirect(`/admin/admissions/applicants/${req.params.id}?err=program_invalid`);
    }
  ...
  rejectApplicant: async (req, res) => {
    ...
    const reason = str(req.body.reason || req.body.notes || "");
