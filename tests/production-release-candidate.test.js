...
test('release package includes a deterministic production runbook and preflight command', () => {
  const pkg = JSON.parse(read('package.json'));
  assert.equal(pkg.scripts['release:preflight'], 'npm run release:readiness && npm run check:hygiene && npm run check:syntax && npm run check:views && npm run check:media && npm run check:csrf && npm test');
