...
test('public pages have resilient image fallback and no fragile picsum or relative img roots', () => {
  const dir = path.join(root, 'views/platform/public');
  const files = fs.readdirSync(dir).filter((f) => f.endsWith('.ejs'));
  let combined = '';
  for (const file of files) combined += read(`views/platform/public/${file}`) + '\n';
  assert.match(combined, /\/js\/public-image-fallback\.js/);
  assert.doesNotMatch(combined, /<img[^>]+src=["']https?:\/\//i);
  assert.match(combined, /\/vendor\/marketing\/google-play-badge\.svg/);
  assert.match(combined, /\/vendor\/marketing\/app-store-badge\.svg/);
  assert.doesNotMatch(combined, /picsum\.photos/i);
  assert.doesNotMatch(combined, /src=["'](?:\.\/)?img\//i);
  const fallback = read('public/js/public-image-fallback.js');
  assert.match(fallback, /feature\.webp/);
  assert.match(fallback, /academylogo\.webp/);
});

test('shared marketing CSS does not depend on third-party avatar image hosts', () => {
  const css = read('public/css/style.css');
  assert.doesNotMatch(css, /w7\.pngwing\.com|encrypted-tbn0\.gstatic\.com|png\.pngtree\.com/);
  assert.match(css, /\/img\/academylogo\.webp/);
});

test('public school/search controllers use local image fallback rather than remote placeholder services', () => {
  ...
  assert.match(schools, /\/img\/hero\.webp/);
  assert.match(search, /\/img\/feature\.webp/);
  assert.doesNotMatch(`${schools}\n${search}`, /picsum\.photos|images\.unsplash\.com/i);
});
...
test('public links opening new tabs cannot control the opener window', () => {
  ...
  for (const file of files) {
    const view = fs.readFileSync(file, 'utf8');
    for (const match of view.matchAll(/<a\b([^>]*\btarget=["']_blank["'][^>]*)>/gi)) {
...
test('school public cover preloads remote media and falls back to local hero asset', () => {
  const client = read('public/js/school-profile-public.js');
  assert.match(client, /fallbackUrl = "\/img\/hero\.webp"/);
  assert.match(client, /const probe = new Image\(\)/);
  assert.match(client, /probe\.onerror = \(\) => applyCover\(fallbackUrl\)/);
  assert.ok(fs.existsSync(path.join(root, 'public/img/hero.webp')));
});
...
test('above-fold marketing hero images are preloaded and fetch-prioritized while repeated art is lazy', () => {
  for (const page of ['about','admissions','benefits','features']) {
    const view = read(`views/platform/public/${page}.ejs`);
    assert.match(view, /rel="preload" as="image" href="\/img\/hero\.webp" fetchpriority="high"/);
    assert.match(view, /src="\/img\/hero\.webp"[^>]*fetchpriority="high"/);
  }
  for (const page of ['about','admissions','benefits']) {
    const view = read(`views/platform/public/${page}.ejs`);
    const heroTags = [...view.matchAll(/<img[^>]+src="\/img\/hero\.webp"[^>]*>/g)].map((m) => m[0]);
    assert.ok(heroTags.length >= 2, page);
    assert.match(heroTags[0], /fetchpriority="high"/);
    for (const tag of heroTags.slice(1)) assert.match(tag, /loading="lazy"/, `${page}: ${tag}`);
  }
});
...
test('Redis isolates session, distributed rate-limit and best-effort cache sockets', () => {
  ...
  assert.match(redis, /rateClient = getRedisClient\("rate"\)/);
  assert.match(redis, /cacheClient = getRedisClient\("cache"\)/);
...
test('tenant public school media has local fallback and lazy below-fold galleries', () => {
  ...
  assert.match(detail, /img \|\| '\/img\/feature\.webp'/);
...
test('booking CSP no longer permits obsolete external marketing image hosts', () => {
  const booking = read('src/controllers/platform/bookingController.js');
  assert.match(booking, /"img-src": \["'self'", "data:", "https:\/\/res\.cloudinary\.com"\]/);
  assert.doesNotMatch(booking, /upload\.wikimedia\.org|developer\.apple\.com/);
});
